...
- Keep handwritten Go `gofmt`-clean and use standard initialisms: `ID`, `URL`, `HTTP`, `DB`.
- Use symbolic `http.Status*` constants. HTTP APIs return JSON consistently, including errors.
...
- Any interpolated SQL fragment must come from a constrained internal constant and include a comment explaining the invariant.
