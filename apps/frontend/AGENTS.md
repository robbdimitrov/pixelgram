...
- The browser must not call the backend directly; it talks only to this SvelteKit server. CSP `connect-src 'self'` enforces the boundary, keeps the session cookie `httpOnly`, keeps the backend API off the public surface, and contains XSS exfiltration to this origin.
...
- Do not add `eslint-disable`, `@ts-ignore`, `@ts-expect-error`, or similar suppressions to get checks passing. Fix the underlying issue unless a documented external API, generated code, or tooling false positive requires a narrow exception.
...
- Do not add a dependency when the platform, SvelteKit, or an existing project primitive handles the requirement. Remove direct dependencies that are no longer imported.
...
- Image bytes are served same-origin by `src/routes/uploads/[key]/+server.ts`, which streams them from the backend through `apiClient` without buffering.
- Use `imageUrl()` for stored image paths. CSP keeps `img-src 'self' data: blob:` so local upload previews can render `data:` URLs and client-side resizing can decode selected files through `blob:` object URLs.
