...
<div class="flex w-full flex-col items-center gap-6 rounded-2xl border border-base-300 bg-base-100 p-6 text-base-content shadow-lg shadow-slate-900/5 sm:px-8 md:flex-row md:items-start">
  <div class="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border border-base-300 shadow-md sm:h-28 sm:w-28">
  ...
  <div class="flex w-full flex-grow flex-col gap-4 text-center md:text-left">
      ...
      {:else}
        ...
        >
          <button
            ...
            class="btn btn-sm h-10 min-h-10 shrink-0 gap-2 rounded-full px-5 font-extrabold shadow-md shadow-slate-900/15 {profileUser.isFollowing ? 'btn-outline' : 'btn-neutral'}"
          >
    ...
    <div class="flex items-center justify-center gap-6 text-sm font-bold text-base-content/70 md:justify-start">
      <a href="/@{profileUser.username}" class="transition-colors hover:text-base-content">
        <strong class="font-black text-base-content">{profileUser.posts}</strong> {pluralize(profileUser.posts, 'post')}
      </a>
      <a href="/@{profileUser.username}/likes" class="transition-colors hover:text-base-content">
        <strong class="font-black text-base-content">{profileUser.likes}</strong> {pluralize(profileUser.likes, 'like')}
      </a>
      <a href="/@{profileUser.username}/followers" class="transition-colors hover:text-base-content">
        <strong class="font-black text-base-content">{profileUser.followers}</strong> {pluralize(profileUser.followers, 'follower')}
      </a>
      <a href="/@{profileUser.username}/following" class="transition-colors hover:text-base-content">
