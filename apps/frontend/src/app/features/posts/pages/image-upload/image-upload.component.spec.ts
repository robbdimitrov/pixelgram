...
vi.mock('../../../../shared/utils/image-resizer', () => ({
...
describe('ImageUploadComponent', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        provideRouter([]),
        {provide: PostService, useValue: mockApiClient},
  ...
  describe('selectFile', () => {
    it('should select file and generate preview if resizing succeeds', async () => {
      ...
      (imageResizer.resizeImageForUpload as import('vitest').Mock).mockResolvedValue(mockFile);
    ...
    it('should set errorMessage if resizing fails', async () => {
      const mockFile = new File([''], 'test.jpg', {type: 'image/jpeg'});
      (imageResizer.resizeImageForUpload as import('vitest').Mock).mockRejectedValue(new Error('Resize failed'));
