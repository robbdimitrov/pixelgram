...
func (h Handler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpx.WriteMessage(w, http.StatusBadRequest, message)
		return
	}
	if !result.Updated {
	...
	if result.UnusedAvatar != "" {
		deleteUploadFile(h.ImageDir, result.UnusedAvatar)
	}
...
func (h Handler) updatePassword(w http.ResponseWriter, r *http.Request, userID, oldPassword, password string) {
	...
	if err != nil || !found {
		httpx.WriteMessage(w, http.StatusBadRequest, "User not found.")
		return
	}
	...
	if err != nil {
		httpx.WriteMessage(w, http.StatusBadRequest, "Bad Request")
		return
	}
...
func deleteUploadFile(imageDir, filename string) {
	...
	_ = os.Remove(filepath.Join(imageDir, filepath.Base(filename)))
}
