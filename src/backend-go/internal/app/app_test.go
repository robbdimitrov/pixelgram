...
func (store *fakeSessionStore) GetSession(_ string) (httpx.Session, error) {
	store.getCalls++
	return store.getSession, store.getErr
}
...
func TestNotFoundJSON(t *testing.T) {
	app := New(Config{}, Dependencies{Sessions: &fakeSessionStore{
		getSession:     httpx.Session{ID: "hashed-session-id", UserID: "1"},
